...
class Curver{
  ...
  void drawFrill(color[][] colorGrid){
    ...
    if(!shrinking){
      stroke(colorGrid[(int)position.x][(int)position.y]);
      fill(0);
      ...
      if(age>1.5f){
        randomL = (polarity)?random(age*10.0f-15.0f):-random(age*10.0f-15.0f);
      ...
      if(age>2.0f){
        fill(255,random(120f,256f),random(120f,256f));
